...
-- NOTE: keymaps wrapped in functions so `telescope.builtin` is only required when
-- the key is pressed (keymaps.lua loads at startup; a bare require here would force
-- telescope to load eagerly and defeat its lazy-loading).
...
vim.keymap.set('n', '<c-p>', tb 'find_files', { desc = 'Search Files' })
...
vim.keymap.set('n', '<leader>F', tb 'live_grep', { desc = '[S]earch by [G]rep' })
vim.keymap.set('n', '<leader>sd', tb 'diagnostics', { desc = '[S]earch [D]iagnostics' })
vim.keymap.set('n', '<leader>sr', tb 'resume', { desc = '[S]earch [R]resume' })
